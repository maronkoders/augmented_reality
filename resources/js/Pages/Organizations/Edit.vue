<template>
  <div>
    ...
    <h2 class="mt-12 text-2xl font-bold">Logs Track</h2>
      ...
      <table class="w-full whitespace-nowrap">
        <tr class="text-left font-bold">
          ...
          <th class="pb-4 pt-6 px-6" >Accessed URL</th>
            ...
            <th class="pb-4 pt-6 px-6" >View more</th>
        ...
        <tr v-for="log in logs.data" :key="log.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
         <td class="border-t">
            <Link class="flex items-center px-6 py-4" :href="`/users_activity/${user.id}/${log.date}`">
              {{ moment(log.date).format('LL') }}
            </Link>
          </td>

           <td class="border-t">
            <Link class="flex items-center px-6 py-4"  tabindex="-1"  :href="`/users_activity/${user.id}/${log.date}`">
              {{ log.ip_address }}
            </Link>
          </td>
           ...
           <!-- <td class="border-t">
            ...
            <Link class="flex items-center px-6 py-4"  tabindex="-1"  :href="`/users_activity/${user.id}/${log.date}`">
             ...
             <td class="w-px border-t">
              <Link class="flex items-center px-4" :href="`/users_activity/${user.id}/${log.date}`" tabindex="-1">
